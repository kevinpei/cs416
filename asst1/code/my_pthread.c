...
int swap_contexts()
{
	...
	case 1:
		...
		// free(ptr->thread);
		// free(ptr);
		if (ptr->thread->pid == scheduler->first_running_queue->thread->pid)
	...
	case 2:
		...
		// free(ptr->thread);
		// free(ptr);
		if (ptr->thread->pid == scheduler->second_running_queue->thread->pid)
		{ // this is the only thread
			__sync_lock_release(&scheduler_running);
			__sync_lock_release(&modifying_queue);
			// setcontext(ptr->thread->context);
	...
	case 3:
		...
		// free(ptr->thread);
		// free(ptr);
		if (ptr->thread->pid == scheduler->third_running_queue->thread->pid)
		{ // this is the only thread
			__sync_lock_release(&scheduler_running);
			__sync_lock_release(&modifying_queue);
			// setcontext(ptr->thread->context);
...
int my_pthread_create(my_pthread_t *thread, pthread_attr_t *attr, void *(*function)(void *), void *arg)
{
	...
	*/
	...
	new_thread->thread->ret_val = NULL;
	new_thread->thread->yield_purpose = 0;
